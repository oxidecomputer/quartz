...
def convert_field(rdlc: RDLCompiler, obj: FieldNode) -> dict:
    ...
    read_se = (
        None if (not obj.get_property("onread")) else obj.get_property("onread").name
    )
    json_obj["se_onread"] = read_se
    write_se = (
        None if (not obj.get_property("onwrite")) else obj.get_property("onwrite").name
    )
    ...
    if obj.get_property("encode") is not None:
        ...
        for name, value in list(
            [(x.name, x.value) for x in obj.get_property("encode")]
        ):
