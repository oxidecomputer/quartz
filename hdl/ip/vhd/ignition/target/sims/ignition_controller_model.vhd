...
begin
    ...
    gen: process
     begin
            case state is
                when IDLE1A =>
                    wait until rising_edge(clk) and mux_valid = '1' and from_ctrlr_data_ready = '1';
                    state <= IDLE1B;
                when IDLE1B =>
    ...
    preencode_data <= '1' & K28_5 when state = IDLE1A or state = IDLE2A else
                      ...
                      '0' & D21_5 when state = IDLE2B else
                      from_ctrlr_data;
    mux_valid <= '1' when state /= DATA else
    ...
    expected_encode <= encode(preencode_data(7 downto 0), preencode_data(8), disp_in);

    disp_reg: process(clk)
     begin
        if rising_edge(clk) then
            if mux_valid and from_ctrlr_data_ready then
                disp_in <= disp_out;
            end if;
        end if;
    end process;
