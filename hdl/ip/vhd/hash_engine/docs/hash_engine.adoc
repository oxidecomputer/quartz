...
Not yet done: `grapefruit` has the same `spi_nor_top` and could take the block the
same way, but is untouched. It still elaborates unchanged -- the new client ports
have defaults on their inputs and its instantiation simply leaves them off.
