...
package spi_nor_tb_pkg is
    ...
    procedure write_instr(signal net : inout network_t; data : std_logic_vector);
    procedure write_dummy(signal net : inout network_t; data : integer);
    procedure write_data(signal net : inout network_t; data : std_logic_vector);
    procedure write_data_size(signal net : inout network_t; data : integer);
...
package body spi_nor_tb_pkg is
    ...
    procedure write_instr(signal net : inout network_t; data : std_logic_vector) is
    begin
        write_bus(net, bus_handle, To_StdLogicVector(INSTR_OFFSET + 16#100#, bus_handle.p_address_length), resize(data, 32));
    end procedure;

    procedure write_dummy(signal net : inout network_t; data : integer) is
    begin
    ...
    procedure write_data_size(signal net : inout network_t; data : integer) is
    begin
        write_bus(net, bus_handle, To_StdLogicVector(DATABYTES_OFFSET+ 16#100#, bus_handle.p_address_length), To_StdLogicVector(data, 32));
    end procedure;

    procedure write_data(signal net : inout network_t; data : std_logic_vector) is
    begin
