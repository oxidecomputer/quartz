...
Closing the remaining gap needs the launch moved a half period earlier -- onto the rising
edge preceding the falling edge used today -- which keeps every bit landing on the same SP5
capture edge but gives it 1.5 periods of flight instead of 1, and needs
`set_multicycle_path -setup 2` so the tools analyse against the right capture edge. That is
worth roughly +7.6ns, taking the path from -3.9ns to about +3.6ns.
