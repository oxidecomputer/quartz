...
begin
    ...
    bench: process
        ...
        alias int_n is << signal th.int_n : std_logic >>;
        alias io is << signal th.io : pca9506_pin_t >>;
        alias io_o is << signal th.io_o : pca9506_pin_t >>;
        alias io_oe is << signal th.io_oe : pca9506_pin_t >>;
    ...
    begin
        ...
        while test_suite loop
            if run("check-read") then
                ...
                read_pca9506_reg(net, IP0_OFFSET, 1, rx_queue, ack_queue);
                check_equal(pop_byte(rx_queue), 0, "Non-reset value read from IP0_OFFSET");
                check_true(is_empty(rx_queue), "rx queue not empty");
                -- check one of the default 1 registers
                read_pca9506_reg(net, IOC0_OFFSET, 1, rx_queue, ack_queue);
                check_equal(pop_byte(rx_queue), 255, "Non-reset value read from IOC0_OFFSET");
                check_true(is_empty(rx_queue), "rx queue not empty");
                ...
                write_pca9506_reg(net, IOC0_OFFSET, tx_queue, ack_queue);
                ...
                for i in 0 to 2 loop
                    -- read it back and check
                    read_pca9506_reg(net, IOC0_OFFSET + i, 1, rx_queue, ack_queue);
                ...
                write_pca9506_reg(net, IOC0_OFFSET, tx_queue, ack_queue, auto_inc => false);
                flush(ack_queue);  -- don't want lingering acks from this transaction
                ...
                check_equal('1', int_n, "irq is asserted incorrectly at reset");
                ...
                check_equal('1', int_n, "irq is asserted incorrectly after i/o toggle while masked");
                io(0)(0) <= '0'; -- set back to default
                wait for 200 ns; -- allow synchronization etc

                -- de-mask this irq
                single_write_pca9506_reg(net, MSK0_OFFSET, X"FE", ack_status);
                wait for 200 ns; -- make sure things propagate.
                check_equal('1', int_n, "irq is asserted incorrectly after de-masking with no toggle");
                io(0)(0) <= '1';  -- toggle pin
                wait for 200 ns; -- allow synchronization etc
                check_equal('0', int_n, "irq did not assert after de-masking and i/o toggle");

                -- return the I/O to its previous state, irq should go away
                io(0)(0) <= '0';  -- toggle pin
                wait for 200 ns; -- allow synchronization etc
                check_equal('1', int_n, "irq still asserted after returning I/O to previous state");
                ...
                read_pca9506_reg(net, IP0_OFFSET, 1, rx_queue, ack_queue);
                check_equal('1', int_n, "irq still asserted (incorrectly) after reading IP0_OFFSET");

                -- now toggle i/o again this should generate a new irq
                io(0)(0) <= '0';  -- toggle pin low again
                wait for 200 ns; -- allow synchronization etc
                check_equal('0', int_n, "irq did not assert after i/o toggle");
                ...
                single_write_pca9506_reg(net, MSK0_OFFSET, X"FE", ack_status);
                -- de-mask i/o (4,0)
                single_write_pca9506_reg(net, MSK4_OFFSET, X"FE", ack_status);
                check_equal('1', int_n, "irq is asserted incorrectly after de-masking with no toggle");
                io(0)(0) <= '1';  -- toggle pin
                io(4)(0) <= '1';  -- toggle pin
                wait for 200 ns; -- allow synchronization etc
                ...
                read_pca9506_reg(net, IP0_OFFSET, 1, rx_queue, ack_queue);
                check_equal('0', int_n, "irq incorrectly de-asserted after reading only IP0_OFFSET");

                -- now read other register, irq should clear
                read_pca9506_reg(net, IP4_OFFSET, 1, rx_queue, ack_queue);
                check_equal('1', int_n, "irq still asserted (incorrectly) after reading IP4_OFFSET");
            -- verify all inputs are read
                ...
                for i in 0 to 4 loop  -- Loop each port
                    for j in 0 to 7 loop  -- Loop each bit
                        -- everything is an input so we should not see any io_oe bits set
                        check_equal(io_oe(i)(j), '0', "io_oe bit " & to_string(j) & " on port " & to_string(i) & " is set for an input");
                        -- Set the bit under test to '1'
                        io(i)(j) <= '1';
                        wait for 200 ns; -- allow synchronization etc
                        -- Read the corresponding input register and check the value
                        read_pca9506_reg(net, IP0_OFFSET + i, 1, rx_queue, ack_queue);
                        flush(ack_queue);  -- don't want lingering acks from this transaction
                ...
                for i in 0 to 4 loop
                    ...
                    single_write_pca9506_reg(net, IOC0_OFFSET + i, X"00", ack_status);

                    for j in 0 to 7 loop
                        check_equal(io_oe(i)(j), '1', "Output enable bit " & to_string(j) & " on port " & to_string(i) & " not set correctly for output pin");
                        -- Set the bit under test to '1'
                        user_int := to_integer(shift_left(std_logic_vector'("00000001"), j));
                        single_write_pca9506_reg(net, OP0_OFFSET + i, to_std_logic_vector(user_int, 8), ack_status);
                        wait for 200 ns; -- allow synchronization etc
                        -- check the output port
                        check_equal(io_o(i)(j), '1', "Output bit " & to_string(j) & " on port " & to_string(i) & " not setting correctly");
                        -- set back to 0
                        single_write_pca9506_reg(net, OP0_OFFSET + i, to_std_logic_vector(0, 8), ack_status);
                        wait for 200 ns; -- allow synchronization etc
                        -- check the output port
                        check_equal(io_o(i)(j), '0', "Output bit " & to_string(j) & " on port " & to_string(i) & " not clearing correctly");
                    end loop;
                ...
                for i in 0 to 4 loop  -- Loop each port
                    -- invert all inputs
                    single_write_pca9506_reg(net, PI0_OFFSET + i, X"FF", ack_status);
