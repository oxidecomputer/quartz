...
entity basic_arbiter is
generic(
  --! Defaults to round-robin, set to `true` to enable LSB
  --! as highest priority requester
...
round_robin_mode: if PRIORITY = false generate
    ...
    requests_masked  <= requests_int and grants_mask when (requests_int and grants_mask) /= 0 else requests_int;
else generate
...
fedge_detects: for i in requests_int'range generate
  req_vec_f_edge(i) <= '1' when requests_int(i) = '0' and requests_last(i) = '1' else '0';
end generate;
req_f_edge <= or req_vec_f_edge; --Unary reduction, active when any bit is active
