...
architecture tb of arb_tb is
begin
  th : entity work.arb_th;
  ...
  begin
    ...
    wait until reset = '0';
    wait for 500 ns;  -- give some time after reset

    
    while test_suite loop
      if run("basic_arbiter test") then
        -- put 3 bits active, show only one bit granted
        requests := 3x"7";
        set_arb(net, requests);
        wait for 20 ns;
        get_grant(net, grants);
