...
architecture rtl of dimm_arb_mux is
    ...
    type mux_reg_t is record
        state       : mux_state_t;
        playback_bits : integer range 0 to 7;
...
begin
    ...
    fpga_i2c_has_bus <= '1' when mux_r.state = FPGA_HAS_GRANT else '0';
    sp5_playback_i2c_has_bus <= '1' when mux_r.state = PLAY_STORED_START or mux_r.state = PLAY_STORED_DATA else '0';
    sp5_i2c_has_bus <= '1' when mux_r.state = CPU_HAS_BUS else '0';

    dimm_i2c_idle <= dimm_scl and dimm_sda;
        ...
        case sample_r.state is 
            ...
            when SAMPLE_DATA =>
                if cpu_scl_redge = '1' then
                    -- sample data bit
                    v.data_bits := shift_left(sample_r.data_bits, 1);
                    v.data_bits(0) := cpu_sda_in;
    ...
    begin
        v := mux_r;
        v.playback_done := '0'; -- single cycle flag
        cpu_start_detected := cpu_scl_in and cpu_sda_fedge;
        cpu_stop_detected := cpu_scl_in and cpu_sda_redge;
        case mux_r.state is 
            when IDLE =>
            ...
            when PLAY_STORED_START =>
                ...
                -- if the CPU is still in the start condition, or is in the scl=0 phase of before the
                -- first bit get's shifted out, we can skip playing back the data
                -- and just hand over the bus immediately. We'll have generated start already here.
                if sample_r.state = SAMPLE_START or 
                   (sample_r.state = SAMPLE_DATA and sample_r.bit_count = 0 and cpu_scl_in = '0') then
                    v.state := CPU_HAS_BUS;
                    v.playback_done := '1';
    ...
    begin
        ...
        if mux_r.state = PLAY_STORED_START or mux_r.state = PLAY_STORED_DATA then
    ...
    playback_scl.oe <= '1' when mux_r.state = PLAY_STORED_START or mux_r.state = PLAY_STORED_DATA else '0';
