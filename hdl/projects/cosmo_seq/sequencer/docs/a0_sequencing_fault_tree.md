...
The IDLE→DDR_BULK_EN transition requires `enable_pend AND upstream_ok`. When `seq_raw_status.hw_sm` reads `0x00` and the system hasn't sequenced, walk this flowchart:

```
seq_api_status.a0_sm == FAULTED (0x0b)?
├─ YES → Previous sequence faulted. See section 8 (MAPO) to identify
│        the cause. Software must clear then re-set a0_en to generate
│        a rising edge (which clears the faulted latch).
│
└─ NO → power_ctrl.a0_en == 1?
         ├─ NO → Software never wrote the enable. Hubris hasn't
         │        requested A0 power-up yet.
         │
         └─ YES → Was there a rising edge on a0_en?
                   (if a0_en was already 1 and software re-wrote 1
                    without first clearing to 0, no edge is detected
                    and enable_pend stays low)
                   ├─ NO → Software must clear a0_en then set it again.
                   │
                   └─ YES → enable_pend is set; upstream_ok must be blocking.
                            Check upstream prerequisites:
                            ┌────────────────────────────────────────────┐
                            │ early_power_rdbks.fan_hsc_west_pg == 1?   │
                            │ early_power_rdbks.fan_hsc_central_pg == 1?│
                            │ early_power_rdbks.fan_hsc_east_pg == 1?   │
                            └────────────────────────────────────────────┘
                            upstream_ok = AND of all three fan HSC PG signals.
                            Whichever is 0 identifies the blocking fan HSC.
```
...
- **8b. DDR bulk MAPO** (monitored from GROUP_A_PG_AND_WAIT onward, same point as Group A)
  - ABCDEF_HSC drops PG — DDR 12V hot-swap controller fault, input 12V brownout/loss
  - GHIJKL_HSC drops PG — same class of fault on the other DDR HSC

- **8c. Group B MAPO** (monitored from GROUP_B_PG_AND_WAIT onward)
  - V1P1_SP5 drops PG — load spike from SP5, regulator thermal, input droop

- **8d. Group C MAPO** (monitored from GROUP_C_PG_AND_WAIT onward)
  - VDDIO_SP5_A0, VDDCR_CPU0, VDDCR_CPU1, or VDDCR_SOC drops PG
  - PMBus VR commanded off externally (e.g., errant Hubris command)

- **8e. `upstream_ok` deasserts** — upstream conditions (fans, thermals, HSCs) lost at any point during sequencing
...
| `rail_pgs` | `abcdef_hsc` (bit 0), `ghijkl_hsc` (bit 1) | Both `1` | Either `0` → DDR bulk HSC dropped PG (fault 8b) |
...
| `early_power_rdbks` | `fan_hsc_*_pg` (bits 1–3) | All `1` | Any `0` → fan HSC lost, caused `upstream_ok` deassert (fault 8e) |
