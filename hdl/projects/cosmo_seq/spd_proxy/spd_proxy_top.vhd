...
architecture rtl of spd_proxy_top is
    ...
    constant SIM_START_HD_TICKS : std_logic_vector(CNTR_SIZE - 1 downto 0) :=
...
begin
    ...
    begin
        if reset then
            ...
            -- we never need to worry about simulating a falling edge on SCL since that will happen
            -- automatically when we cut over to the CPU bus where SCL and SDA are both low already.
            scl_sim <= '1';
    ...
    ctrlr_has_int_mux   <= not need_start or ctrlr_idle = '0';
    ...
    cpu_has_mux     <= cpu_busy and ctrlr_idle = '1' and not need_start;
